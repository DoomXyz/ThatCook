...
const rejectSchedule = async (ScheduleID, transaction) => {
  try {
    ...
    if (appointment.AppointmentStatus === 'CONF') {
      await db.Appointment.update({ AppointmentStatus: 'PEND' }, { where: { AppointmentID: schedule.AppointmentID }, transaction });
    }
    ...
    for (const conflictingApp of conflictingAppointments) {
      ...
      if (scheduleStart < conflictingEnd && scheduleEnd > conflictingStart) {
        await db.Appointment.update({ AppointmentStatus: 'PEND' }, { where: { AppointmentID: conflictingApp.AppointmentID }, transaction });
      }
    }
    await db.Schedule.destroy({
      where: { ScheduleID },
