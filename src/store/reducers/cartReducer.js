  ...
  switch (action.type) {
    ...
    case actionTypes.ADD_TO_CART: {
      ...
      newItems.forEach((newItem) => {
        const existingItemIndex = updatedCart.findIndex((item) => item.ProductID === newItem.ProductID && item.ProductDetailID === newItem.ProductDetailID);
    ...
    case actionTypes.REMOVE_FROM_CART:
      return {
        ...state,
        cartItems: state.cartItems.filter((item) => item.ProductID !== action.data.ProductID || item.ProductDetailID !== action.data.ProductDetailID),
    ...
    case actionTypes.MERGE_DETAIL: {
      ...
      const existingItemIndex = newCartItems.findIndex((item) => item.ProductID === action.data.ProductID && item.ProductDetailID === action.data.ProductDetailID2);
      if (existingItemIndex !== -1) {
        ...
        newCartItems.push({
          ...
          ItemQuantity: action.data.newItemQuantity,
          ItemPrice: state.cartItems.find((item) => item.ProductID === action.data.ProductID && item.ProductDetailID === action.data.ProductDetailID1)?.ItemPrice || 0,
        });
      }
      return {
        ...state,
