...
class EditProductModal extends Component {
  ...
  handleAddImage = (e) => {
    ...
    this.setState((prevState) => ({
      allImages: [
        ...prevState.allImages,
        { ImageID: Date.now(), Image: preview, file },
      ],
    }));
  };
  ...
  handleDetailChange = (index, field, value) => {
    this.setState((prevState) => {
      ...
      if (field === "Stock") {
        newDetails[index].DetailStatus =
          parseInt(value) > 0 ? newDetails[index].DetailStatus : "OUT";
      }
  ...
  handleCancelDetail = () => {
    this.setState((prevState) => {
      if (
        prevState.isAddingDetail &&
        prevState.editingField === prevState.loadedProductDetailInfo.length - 1
      ) {
        return {
          loadedProductDetailInfo: prevState.loadedProductDetailInfo.slice(
            0,
            -1
          ),
          editingField: null,
  ...
  checkValidateDetail = (index) => {
    ...
    if (isNaN(item.Stock) || parseInt(item.Stock) < 0)
      return {
        errCode: -1,
        errMessage: `Số lượng tồn tại dòng ${index + 1
    ...
    if (parseInt(item.Stock) === 0 && item.DetailStatus === "AVAIL")
      return {
        errCode: -1,
        errMessage: `Số lượng tồn tại dòng ${index + 1
          } bằng 0, không thể chọn trạng thái Còn hàng!`,
      };
    if (
      item.ExtraPrice !== "" &&
      item.ExtraPrice !== undefined &&
      (isNaN(item.ExtraPrice) || parseFloat(item.ExtraPrice) < 0)
    ) {
      return {
        errCode: -1,
    ...
    if (
      ...
      (isNaN(item.Promotion) ||
        parseFloat(item.Promotion) < 0 ||
        parseFloat(item.Promotion) > 100)
    ) {
      return {
        errCode: -1,
    ...
    if (!item.DetailStatus)
      return {
        errCode: -1,
        errMessage: `Trạng thái chi tiết tại dòng ${index + 1
          } không được để trống!`,
      };
  ...
  checkValidateProduct = () => {
    ...
    if (!productname)
      return { errCode: -1, errMessage: "Tên sản phẩm không được để trống!" };
    const nameRegex = /^(?=.*[A-Za-zÀ-ỹ]).{2,100}$/;
    if (!nameRegex.test(productname.trim()))
      return { errCode: -1, errMessage: "Tên sản phẩm không hợp lệ!" };

    if (!producttype)
      return { errCode: -1, errMessage: "Vui lòng chọn loại sản phẩm!" };

    if (!productprice)
      return { errCode: -1, errMessage: "Giá bán không được để trống!" };
    if (isNaN(productprice) || parseFloat(productprice) <= 0)
      return { errCode: -1, errMessage: "Giá bán phải lớn hơn 0!" };

    if (!productdescription)
      return { errCode: -1, errMessage: "Mô tả sản phẩm không được để trống!" };

    if (allImages.length === 0)
      return { errCode: -1, errMessage: "Vui lòng thêm ít nhất 1 hình ảnh!" };
    if (allImages.length > 5)
      return { errCode: -1, errMessage: "Tối đa 5 hình ảnh!" };
      ...
      const productInfo = {
        ...
        ProductDetail: loadedProductDetailInfo.map((item) => ({
          ...
          ExtraPrice: item.ExtraPrice
            ? parseFloat(item.ExtraPrice).toFixed(2)
            : "0.00",
          Promotion: item.Promotion
            ? parseFloat(item.Promotion).toFixed(2)
            : "0.00",
          ...
          <div className="modal-content">
            <div className="modal-content-add-img">
              ...
              <div className="f">
                {allImages.map((img, index) => (
                  <div
                    key={img.ImageID}
                    className="modal-content-add-img-item f"
                  >
                    <img
                      src={img.Image}
                      alt={`Hình ảnh ${index === 0 ? "chính" : "phụ"}`}
                    />
                    <button
            ...
            <div className="modal-content-add-type-price">
              ...
              <div className="product-detail-table">
                <table className="table">
                  ...
                  <tbody>
                    {loadedProductDetailInfo &&
                      loadedProductDetailInfo.length > 0 ? (
                        ...
                        <tr key={item.ProductDetailID}>
                          ...
                          <td>
                            {editingField === index ? (
                              ...
                              codeDetailStatus.find(
                                (status) => status.Code === item.DetailStatus
            ...
            <div className="modal-content-add-info">
              ...
              <textarea
                ...
                onChange={(e) =>
                  this.handleInputChange(e, "productdescription")
                }
              />
            </div>
          </div>
