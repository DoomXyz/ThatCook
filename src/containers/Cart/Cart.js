...
class Cart extends Component {
  ...
  handleCheckCartQuantity = async () => {
    ...
    try {
      ...
      for (let i = 0; i < updatedCartInfo.length; i++) {
        ...
        const productInfo = loadedCartDetailInfo.find(
          (product) =>
            product.ProductID === cartItem.ProductID &&
            product.ProductDetailID === cartItem.ProductDetailID
        );
  ...
  handleChangeProductDetail = async (productid, productdetailid1, productdetailid2) => {
    ...
    if (loadedCartDetailInfo && loadedCartDetailInfo.length > 0) {
      isExistDetail = loadedCartDetailInfo.some(item =>
        item.ProductID === productid && item.ProductDetailID === productdetailid2
      );
    }
    if (!isExistDetail) {
      if (isLoggedIn) {
        ...
        const confirmMerge = () =>
          new Promise((resolve) => {
            toast(
              <div>
                <p>
                  Bạn đang có chung sản phẩm nhưng khác loại trong giỏ hàng, xác
                  nhận gộp sản phẩm?
                </p>
                <button
                  className="toast-confirm-btn"
                          ...
                          <tr key={index} className="cart-content-left-item">
                            ...
                            <td>
                              <select
                                ...
                                onChange={(e) =>
                                  this.handleChangeProductDetail(
                                    item.ProductID,
                              ...
                              >
                                {loadedCartDetailList.find(detail => detail.ProductID === item.ProductID)?.DetailList?.length > 0 ? (
                                  loadedCartDetailList.find(detail => detail.ProductID === item.ProductID).DetailList.map(
                                    (detail, index) => (
                            ...
                            <td>
                              <button
                                onClick={() => this.handleDecreaseQuantity(item.ProductID, item.ProductDetailID, item.ItemQuantity - 1)}
                              ...
                              />
                              <button
                                onClick={() => this.handleAddQuantity(item.ProductID, item.ProductDetailID, item.ItemQuantity + 1)}
                              >
                                +
                              </button>
                            </td>
                            ...
                            <td>
                              <button
                                onClick={() => this.handleRemoveFromCart(item.ProductID, item.ProductDetailID)}
                                disabled={disabledRemoveButton}
                              >
                                <p>x</p>
                              </button>
                  ...
                  <table>
                    ...
                    <tbody>
                      <tr>
                        <td>TỔNG SẢN PHẨM:</td>
                        <td>
                          {loadedCartDetailInfo.length > 0
                            ? `${loadedCartDetailInfo.length} sản phẩm`
                            : ""}
                        </td>
                      </tr>
                      <tr>
                  ...
                  <div className="cart-content-right-button">
                    ...
                    <div>
                      <button
                        ...
                        disabled={loadedCartDetailInfo.length === 0}
                      >
                        THANH TOÁN
                      </button>
                    </div>
